				...
				<div class="row gy-4 portfolio-container">
					...
					if (isset($_GET['query'])) {
						...
						$sql = "SELECT 
							...
							INNER JOIN book_publisher ON book_details.Book_id = book_publisher.Book_id
							INNER JOIN publisher ON book_publisher.Publisher_id = publisher.Publisher_id
							WHERE 
							(
								author.Author_Name LIKE '%" . $query . "%'
								OR book_details.Book_Title LIKE '%" . $query . "%'
								OR book_details.Publication_Year LIKE '%" . $query . "%'
								OR edition.Edition_Name LIKE '%" . $query . "%'
								OR book_details.ISBN LIKE '%" . $query . "%'
						...
						// where book_details.Book_id = book_author.Book_id
						// and author.Author_id = book_author.Author_id
						// and edition.Edition_id = book_edition.Edition_id 
						// and publisher.Publisher_id = book_publisher.Publisher_id
